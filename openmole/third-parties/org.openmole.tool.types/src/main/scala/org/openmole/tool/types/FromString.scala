...
object FromString {
  ...
  implicit def doubleFromString = FromString[Double](_.toDouble)
  implicit def fileFromString = FromString[java.io.File](s ⇒ new java.io.File(s))
  implicit def intFromString = FromString[Int](_.toInt)
  implicit def longFromString = FromString[Long](_.toLong)
  implicit def floatFromString = FromString[Float](_.toFloat)
  implicit def bigDecimalFromString = FromString[BigDecimal](s ⇒ BigDecimal(s, MathContext.DECIMAL128))
  implicit def bigIntFromString = FromString[BigInt](s ⇒ BigInt(s))
  implicit def javaBigDecimalFromString = FromString[java.math.BigDecimal](s ⇒ BigDecimal(s, MathContext.DECIMAL128).bigDecimal)
  implicit def javaBigIntegerFromString = FromString[java.math.BigInteger](s ⇒ BigInt(s).underlying)
  implicit def stringFromString = FromString[String](identity)
  implicit def booleanFromString = FromString[Boolean](_.toBoolean)
  implicit def timeFromString = FromString[Time](_.toTime.get)
  implicit def memoryFromString = FromString[Information](_.toInformation.get)
