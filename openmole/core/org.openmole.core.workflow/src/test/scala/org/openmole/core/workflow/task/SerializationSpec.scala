...
class SerializationSpec extends flatspec.AnyFlatSpec with matchers.should.Matchers:
  ...
  "Task" should "be the same after serialization and deserialization" in:
    ...
    val mole: Mole = t
    val rt = MoleExecution.runtimeTasks(mole, Sources.empty, Hooks.empty, _ => TaskExecutionBuildContext(cache, MoleExecution.BuildEventHandler())).head._2
  ...
  "MoleTask" should "be able to run after serialization and deserialization" in :
    ...
    val rt =
      MoleExecution.runtimeTasks(MoleTask(t), Sources.empty, Hooks.empty, _=> TaskExecutionBuildContext(cache, MoleExecution.BuildEventHandler())).head
