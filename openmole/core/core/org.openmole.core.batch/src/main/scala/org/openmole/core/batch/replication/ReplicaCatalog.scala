...
object ReplicaCatalog extends Logger {
  ...
  lazy val database = {
    ...
    val db = Database.forDriver(driver = new org.h2.Driver, url = s"jdbc:h2:tcp://localhost:${info.port}/${DBServerInfo.base}/${DBServerInfo.dbName};MV_STORE=FALSE", user = info.user, password = info.password)
    db.withSession {
      s ⇒
