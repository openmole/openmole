...
trait IBatchServiceControl {
  ...
  def registerRessouce(ressource: BatchServiceDescription, usageControl: IUsageControl, failureControl: IFailureControl)

  def usageControl(ressource: BatchServiceDescription): IUsageControl
    
  def failureControl(ressource: BatchServiceDescription): Option[IFailureControl]
