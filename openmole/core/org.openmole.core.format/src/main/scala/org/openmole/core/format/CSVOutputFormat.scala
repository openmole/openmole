...
object CSVOutputFormat:
  ...
  implicit def format[MD](using default: CSVOutputFormatDefault[MD]): OutputFormat[CSVOutputFormat, MD] = new OutputFormat[CSVOutputFormat, MD] {
    override def write(executionContext: OutputFormat.FormatExecutionContext)(f: CSVOutputFormat, output: WritableOutput, content: OutputFormat.OutputContent, method: MD, append: Boolean): FromContext[Unit] = FromContext { p ⇒
      import p._
