...
object Market {
  ...
  def entries = Seq(
    MarketRepository(
      ...
      MarketEntry("Sensitivity Analysis (Morris)", "sensitivity/morris", Seq(netlogo, sensitivity, simulation)),
      MarketEntry("Sensitivity Analysis (Saltelli)", "sensitivity/saltelli", Seq(sensitivity, simulation)),
