...
${ul(
    ...
    li(html"""${code{"stopCondition"}} String, a stoping condition for your simulation in GAMA, for instance ${code("(cycle=10 or nb_preys<10)")}, it must be set if no finalStep is set, in case both are set only the stopping condition in taken into accoount"""),
    ...
    li(html"""${code{"parentExperiment"}} String, the name of a parent experiment for the generated OpenMOLE experiment"""),
