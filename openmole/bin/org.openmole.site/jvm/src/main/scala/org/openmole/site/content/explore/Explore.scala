...
Therefore, every model must be questioned in order to fully understand the extent of its capabilities:
...
$br
These are just some of the many questions a model developer has to answer in order to really know and understand their model, and OpenMOLE built-in methods are designed to help you answer some of these questions.
