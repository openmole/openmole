...
object Runtime extends BaseDefaults {
  ...
  val runtime = OsgiProject("runtime", singleton = true) dependsOn (Core.model, Core.batch, Core.serializer, Misc.logging, Misc.eventDispatcher, Misc.exception) settings
    (includeOsgi, bundleType += "runtime", libraryDependencies ++= Seq(scalaLang, scopt, equinoxCommon, equinoxApp))

  val daemon = OsgiProject("daemon", singleton = true, imports = Seq("*")) dependsOn (Core.model, Core.model, Core.batch, Misc.workspace,
