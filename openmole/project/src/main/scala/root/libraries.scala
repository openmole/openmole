...
object Libraries extends Defaults(Apache) {
  ...
  lazy val scalabc = OsgiProject("fr.irstea.scalabc", privatePackages = Seq("!scala.*", "*")) settings (libraryDependencies += "fr.irstea" %% "scalabc" % "0.3-SNAPSHOT", bundleType := Set("plugin"))
