...
object Libraries extends Defaults(Apache) {
  ...
  val gridscaleVersion = "1.72-SNAPSHOT"
  ...
  lazy val xstream = OsgiProject("com.thoughtworks.xstream", buddyPolicy = Some("global"), privatePackages = Seq("!scala.*", "*")) settings
    (libraryDependencies ++= Seq("com.thoughtworks.xstream" % "xstream" % "1.4.7",
  ...
  lazy val mgo = OsgiProject("fr.iscpif.mgo") settings (libraryDependencies += "fr.iscpif" %% "mgo" % "1.73", bundleType := Set("plugin"))
