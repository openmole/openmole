...
class Activator extends BundleActivator {
  ...
  override def start(context: BundleContext): Unit = {
    ...
    val keyWords: Vector[HighLight] =
      Vector(
        PatternHighLight(objectName(MasterSlave)),
        PatternHighLight(objectName(Skip)),
        PatternHighLight(objectName(Sequence)),
        PatternHighLight(objectName(Switch)),
        PatternHighLight(objectName(While))
      )
