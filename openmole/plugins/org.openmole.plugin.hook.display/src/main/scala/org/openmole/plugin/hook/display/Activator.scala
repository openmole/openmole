...
class Activator extends BundleActivator {
  ...
  override def start(context: BundleContext): Unit = {
    ...
    val keyWords: Vector[HighLight] =
      Vector(
        HookHighLight("DisplayHook"),
        HookHighLight("ToStringHook")
      )

    PluginRegistry.register(this, Vector(this.getClass.getPackage), highLight = keyWords, preferenceLocation = PreferenceLocation.list(DisplayHook))
  }
}
