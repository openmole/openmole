...
object GAMALegacyTask:
  ...
  def checkXML(inputXMLString: String, experiment: String, frameRate: Option[Int], mapped: MappedInputOutputConfig) =
    ...
    def validateOutputs =
      val acceptedOutputsTypes = GAMALegacyTask.acceptedOutputType(frameRate.isDefined)
  ...
  def apply(
    ...
    ExternalTask.build("GAMATask"): buildParameters =>
      ...
      val inputFilePath = s"${GAMALegacyTask.workspaceDirectory}/_inputs_openmole_.xml"
      ...
      ExternalTask.execution: executionParameters =>
        ...
        val parsedInputXML =
          ...
          GAMALegacyTask.modifyInputXML(inputMap, finalStep.from(context), seedValue, frameRate.option).transform(parsedInputXML)
