...
package object bounds {
  ...
  implicit def arrayOfTuplesIsBounds[T: Manifest] = new Bounds[Array[(T, T)], Array[T]] {
    override def min(domain: Array[(T, T)]) = FromContext.value(domain.unzip._1.toArray)
    override def max(domain: Array[(T, T)]) = FromContext.value(domain.unzip._2.toArray)
  }
  ...
  implicit def arrayOfStringTuplesIsBounds[T: FromString: Manifest] = new Bounds[Array[(String, String)], Array[T]] {
