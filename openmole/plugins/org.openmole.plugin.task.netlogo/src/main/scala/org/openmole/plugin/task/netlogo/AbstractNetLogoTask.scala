    ...
    try
      ...
      then Variable.unsecureUntyped(mapped.v, AbstractNetLogoTask.cast(outputValue, runtimeClass))
        ...
        val netLogoCollection: util.AbstractCollection[Any] =
          ...
          then
            println("cast array")
            outputValue.asInstanceOf[AbstractCollection[Any]]
          else
            println("not cast")
...
trait AbstractNetLogoTask extends Task with ValidateTask:
  ...
  override def validate = Validate: p =>
    ...
    def scriptExists =
      workspace match
        case s: AbstractNetLogoTask.Workspace.Script if !s.script.exists => Seq(UserBadDataError(s"File ${s.script} doesn't exit"))
        case d: AbstractNetLogoTask.Workspace.Directory =>
          ...
          then Seq(UserBadDataError(s"File ${d.script} doesn't exists in workspace directory ${d.directory}"))
          else Seq()
        case _ => Seq()
