...
object SSHEnvironment extends JavaLogger:
  ...
  def submit[S: HierarchicalStorageInterface](environment: BatchEnvironment, runtimeSetting: Option[RuntimeSetting], batchExecutionJob: BatchExecutionJob, storage: S, space: StorageSpace, jobService: SSHJobService[?])(using services: BatchEnvironment.Services, priority: AccessControl.Priority) =
    submitToCluster(
      environment,
      runtimeSetting,
      batchExecutionJob,
