...
trait SharedStorage extends SSHService { js ⇒
      ...
      case None ⇒ sharedFS.withToken { implicit token ⇒
        ...
        val (workdir, scriptName) = {
          ...
          try {
            ...
            sharedFS.uploadGZ(script, remoteScript, options = TransferOptions(raw = true))
            (workdir, scriptName)
          }
  ...
  protected def buildScript(serializedJob: SerializedJob) = {
    ...
    val remoteScript = try {
      ...
      sharedFS.withToken { sharedFS.uploadGZ(script, remoteScript, options = TransferOptions(raw = true))(_) }
      remoteScript
    }
    finally script.delete
    (remoteScript, result)
