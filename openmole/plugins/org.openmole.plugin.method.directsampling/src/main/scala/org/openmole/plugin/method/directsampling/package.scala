...
package object directsampling {
  ...
  def DirectSampling[P](
    ...
    val wrapped = wrapPuzzle(evaluation, sampling.prototypes.toSeq, evaluation.outputs)
    ...
    aggregation.option match {
      case Some(aggregation) ⇒
        ...
        OutputEnvironmentPuzzleContainer(p, wrapped.evaluationPuzzle.last, wrapped.delegate)
      case None ⇒
        ...
        val p = (explorationCapsule -< preCapsule -- (evaluation when condition) -- afterSlot) &
