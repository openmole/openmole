...
object AggregateTask {
  ...
  object AggregateVal {
    ...
    implicit def fromEvaluate[A, B: ValTag, V[_]: FromArray](a: Evaluate[Val[A], V[A] => B]): AggregateVal[A, B] = AggregateVal(a, a.value.withType[B])
    ...
    implicit def fromAsEvaluate[A, B: ValTag, V[_]: FromArray](as: As[Evaluate[Val[A], V[A] => B], Val[B]]): AggregateVal[A, B] = AggregateVal(as.value, as.as)
    ...
    implicit def fromAsStringEvaluate[A, B: ValTag, V[_]: FromArray](as: As[Evaluate[Val[A], V[A] => B], String]):  AggregateVal[A, B] = AggregateVal(as.value, Val[B](as.as))

    implicit def fromVal[A](v: Val[A]): AggregateVal[A, Array[A]] = AggregateVal[A, Array[A], Array](Aggregate(v, identity), v.toArray)
    implicit def fromAsVal[A: ValTag](v: As[Val[A], Val[Array[A]]]): AggregateVal[A, Array[A]] = AggregateVal[A, Array[A], Array](Aggregate[Val[A], Array[A] => Array[A]](v.value, identity), v.as)
    implicit def fromAsValString[A: ValTag](v: As[Val[A], String]): AggregateVal[A, Array[A]] = AggregateVal[A, Array[A], Array](Aggregate[Val[A], Array[A] => Array[A]](v.value, identity), Val[Array[A]](v.as))

    implicit def fromScalaCode[A: ValTag](v: Evaluate[Val[A], ScalaCode | String]): AggregateVal[A, A] = 
    ...
    implicit def fromAsScalaCode[A, B: ValTag](v: As[Evaluate[Val[A], ScalaCode | String], Val[B]]): AggregateVal[A, B] = 
      val fromContext =
    ...
    def apply[A, B, V[_]: FromArray](a: Evaluate[Val[A], V[A] => B], _outputVal: Val[B]): AggregateVal[A, B] = new AggregateVal[A, B]:
