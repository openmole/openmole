...
object SensitivityMorris {
  ...
  object MorrisAggregation {
    ...
    def apply[T](
      ...
      .set (
        ...
        (inputs, outputs) ++= ScalableValue.prototypes(modelInputs).map(_.array) ++ modelOutputs.map(_.array),
  ...
  object MorrisSampling extends JavaLogger {
    ...
    given IsSampling[MorrisSampling] = s =>
      Sampling(
        s.apply(),
        s.outputs,
        s.inputs,
    ...
    def trajectoryBuilder(delta: Double, order: List[Int], direction: List[Int], seed: Array[Double],
      ...
      if order.isEmpty
      then (accPoints.reverse, accDeltas.reverse)
      else
  ...
  given ExplorationMethod[SensitivityMorris, Method] = m =>
    given DefinitionScope = m.scope
