  ...
  def !(msg: JobMessage)(using services: BatchEnvironment.Services): Unit = services.threadProvider.virtual: () =>
    ...
    msg match
      ...
      case Submitted(job, environment, bj) =>
        killOr(environment, job.storedJob, Kill(job, environment, Some(bj), finished = false)) { () => self ! Delay(Refresh(job, environment, bj, bj.updateInterval.minUpdateInterval), bj.updateInterval.minUpdateInterval) }
