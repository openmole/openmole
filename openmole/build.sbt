...
lazy val dockerBin = Project("docker", binDir / "docker") enablePlugins (sbtdocker.DockerPlugin) settings(
  ...
  docker / dockerfile := new Dockerfile {
    ...
    runRaw(
      """cp /etc/apt/sources.list /etc/apt/sources.list.d/unsable.list && sed -i "s/testing/unstable/g" /etc/apt/sources.list.d/unsable.list && sed -i "/unstable-security/d" /etc/apt/sources.list.d/unsable.list && \
       apt-get update && \
