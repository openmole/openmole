   ...
   private final void expandSearchWindow(int radius)
   {
      if (radius > 0)  // if radius <=0 then no search is necessary, use the current search window
      {
         ...
         for (int cell=0; cell<windowCells.size(); cell++)
         {
            ...
            {
               // Either extend full search radius or some fraction until edges of org.openmole.tool.dwt.matrix are met.
               ...
               {
                  // Expand the window only to the edge of the org.openmole.tool.dwt.matrix.
            ...
            if (currentCell.getRow() != maxJ())  // move up if possible
            {
               ...
               if (targetRow <= maxJ())
                  markVisited(targetCol, targetRow);  // radius does not go past the edges of the org.openmole.tool.dwt.matrix
               else
               {
                  // Expand the window only to the edge of the org.openmole.tool.dwt.matrix.
