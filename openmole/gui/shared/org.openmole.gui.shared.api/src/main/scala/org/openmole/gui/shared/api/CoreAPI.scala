...
//  implicit lazy val saveFileRequestSchema: JsonSchema[(SafePath, String, Option[String], Boolean)] = genericJsonSchema
  ...
  val listRecursive: ErrorEndpoint[(SafePath, Option[String]), Seq[(SafePath, Boolean)]] =
    errorEndpoint(post(path / prefix / "file" / "list-recursive", jsonRequest[(SafePath, Option[String])]), ok(jsonResponse[Seq[(SafePath, Boolean)]]))
  ...
  val omrContent: ErrorEndpoint[SafePath, GUIOMRContent] =
    errorEndpoint(post(path / prefix / "file" / "omr" / "content", jsonRequest[SafePath]), ok(jsonResponse[GUIOMRContent]))
...
//  lazy val allStatesResponseSchema: JsonSchema[(Seq[(ExecutionId, ExecutionInfo)], Seq[OutputStreamData])] = genericJsonSchema
  ...
  val executionOutput: ErrorEndpoint[(ExecutionId, Int), String] =
    errorEndpoint(post(path / prefix / "execution" / "output", jsonRequest[(ExecutionId, Int)]), ok(jsonResponse[String]))
...
//  def clearEnvironmentErrors(environmentId: EnvironmentId): Unit
  val clearEnvironmentErrors: ErrorEndpoint[EnvironmentId, Unit] =
    errorEndpoint(post(path / prefix / "execution" / "clear-environment-error", jsonRequest[EnvironmentId]), ok(jsonResponse[Unit]))
...
//  def runningErrorEnvironmentData(environmentId: EnvironmentId, lines: Int): EnvironmentErrorData
  val listEnvironmentErrors: ErrorEndpoint[(EnvironmentId, Int), Seq[EnvironmentError]] =
    errorEndpoint(post(path / prefix / "execution" / "list-environment-error", jsonRequest[(EnvironmentId, Int)]), ok(jsonResponse[Seq[EnvironmentError]]))
