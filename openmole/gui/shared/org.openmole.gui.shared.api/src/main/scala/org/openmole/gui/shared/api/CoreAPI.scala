...
//  implicit lazy val saveFileRequestSchema: JsonSchema[(SafePath, String, Option[String], Boolean)] = genericJsonSchema
  ...
  val listFiles: ErrorEndpoint[(SafePath, FileSorting), ListFilesData] =
    errorEndpoint(post(path / "file" / "list", jsonRequest[(SafePath, FileSorting)]), ok(jsonResponse[ListFilesData]))
