...
object JSPack:
  ...
  def esBuild(entryJSFile: File,
              nodeModulesFile: File,
              esBuildOutput: File)(using newFile: TmpDirectory, networkService: NetworkService) = {
    newFile.withTmpDir: targetDir =>
      org.openmole.tool.archive.Zip.unzip(nodeModulesFile, targetDir, overwrite = true)
      EsBuild.run(entryJSFile, targetDir, esBuildOutput)
  }
