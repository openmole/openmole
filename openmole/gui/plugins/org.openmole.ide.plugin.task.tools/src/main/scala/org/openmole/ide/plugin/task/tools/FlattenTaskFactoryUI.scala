...
class FlattenTaskFactoryUI extends TaskFactoryUI {
  override def toString = "Flatten"

  def buildDataUI = new FlattenTaskDataUI

  def buildDataProxyUI(task: ITask, uiMap: PuzzleUIMap) = {
    val t = SceneFactory.as[FlattenTask](task)
    uiMap.task(task, x ⇒ new FlattenTaskDataUI(t.name, t.toFlatten.toList.map { p ⇒ (uiMap.prototypeUI(p._1).get, uiMap.prototypeUI(p._2).get) }))
  }
