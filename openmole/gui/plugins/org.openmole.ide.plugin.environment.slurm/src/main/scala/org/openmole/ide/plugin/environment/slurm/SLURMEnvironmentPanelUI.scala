...
class SLURMEnvironmentPanelUI(pud: SLURMEnvironmentDataUI)(implicit val i18n: ResourceBundle = ResourceBundle.getBundle("help", new Locale("en", "EN"))) extends PluginPanel("fillx,wrap 2", "", "") with EnvironmentPanelUI {
  ...
  val components = List(("Settings",
    ...
    new PluginPanel("wrap 2") {
      ...
      contents += new PluginPanel("wrap 2") {
        ...
        contents += (new Label("GRES"), "gap para")
        contents += gresTextField

        contents += (new Label("Constraints"), "gap para")
        contents += constraintsTextField
