...
trait ServerAPI:
  ...
  def extractArchive(path: SafePath)(using BasePath): Future[Unit]
  ...
  def upload(fileList: FileList, destinationPath: SafePath, fileTransferState: ProcessState ⇒ Unit = _ => ())(using BasePath): Future[Seq[String]]
