...
class GUIDocPanel extends ModalPanel {
  ...
  val resourcesContent = tags.div(
    ...
    div("centerImg")(img(src := "img/fileManagement.png", alt := "fileManagement")),
    ...
    div(spacer20, "The content of the current directory is listed at the bottom. Each row gives the name and size of each file or folder. Folders are marked by a ", tags.div(`class` := "dir bottom-5"), ". A ", span(glyph_plus), " symbol indicates that the folder is not empty."),
    div(
      spacer20,
      "Hovering a file or a folder with the mouse pointer triggers new actions:",
      ul(
        li(glyph_download +++ glyphStyle, " download the hovered file or directory (as an archive for the latter) to the local machine."),
        li(glyph_edit +++ glyphStyle, " rename the hovered file or directory. An input field appears: just type the new name and press ", tags.em("enter"), " to validate."),
        li(glyph_trash +++ glyphStyle, " delete the hovered file or directory."),
        li(glyph_archive +++ glyphStyle, " uncompress the hovered file (appears only in case of archive files (", tags.em(".tgz"), " or ", tags.em(".tar.gz"), ").")
      )
    ),
        ...
        Rx {
          tags.div(
            ...
            for (entry ← entries) yield {
              ...
              tags.div(
                if (isSelected) docEntry else docTitleEntry,
                tags.span(
                  docTitleEntry,
                  onclick := { () ⇒
                    {
