...
class OpenMOLERESTServerAPI(fetch: Fetch) extends ServerAPI:
  ...
  override def executionState(line: Int, ids: Seq[ExecutionId]): Future[Seq[ExecutionData]] = fetch.future(_.executionState(line, ids).future)
  ...
  override def compileScript(script: SafePath): Future[Option[ErrorData]] = fetch.future(_.compileScript(script).future, timeout = Some(600 seconds), warningTimeout = None)
  override def launchScript(script: SafePath, validate: Boolean): Future[ExecutionId] = fetch.future(_.launchScript(script, validate).future, timeout = Some(120 seconds), warningTimeout = Some(30 seconds))
  ...
  override def isAlive(): Future[Boolean] = fetch.future(_.isAlive(()).future, timeout = Some(3 seconds), warningTimeout = None)
