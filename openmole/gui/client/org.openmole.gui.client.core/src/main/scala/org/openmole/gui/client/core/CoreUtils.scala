...
object CoreUtils {
  ...
  def listFiles(safePath: SafePath, fileFilter: FileSorting = FileSorting())(using api: ServerAPI, path: BasePath): Future[FileListData] = api.listFiles(safePath, fileFilter)
