      ...
      yield
        event match
          case e: NotificationEvent.MoleExecutionFinished =>
            ...
            NotificationLine(NotificationLevel.Info, title, org.openmole.gui.client.ext.Utils.errorTextArea(body), DataUtils.randomId, serverId = Some(NotificationEvent.id(event)))
