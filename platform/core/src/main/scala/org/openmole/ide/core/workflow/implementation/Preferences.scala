...
object Preferences {
  ...
  def register(cat: MoleConcepts.Concept, coreClass: Class[_],prop: Properties): Unit= {
    println("REQISTES : " + Class.forName(prop.getProperty(PropertyManager.IMPL)))
    registerModel(cat,coreClass,Class.forName(prop.getProperty(PropertyManager.IMPL)))
    registerProperties(cat,coreClass,prop)
  }
  
  def registerModel(cat: MoleConcepts.Concept, coreClass: Class[_], modelClass: Class[_])= {
    println("ModelClasse : " +modelClass)
...
//  }
  ...
  def model(cat: MoleConcepts.Concept, coreClass: Class[_])= {
    register
    println("MODEL :: " + cat + ", " + coreClass)
    println("MODEL RES :: " + models(cat).closestRegistred(coreClass).size)
    println("MODEL RES :: " + models(cat).closestRegistred(coreClass).head)
    models(cat).closestRegistred(coreClass).head
    //iterator.next 
  }
  ...
  def prototypeTypeClasses: Set[Class[_]]= {
    println("to be registered  ")
    register
    println("register over "+ models.size)
