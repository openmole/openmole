...
class ManagementPanel(manager: IManager) extends JPanel(new BorderLayout) with ListSelectionListener with ActionListener{
  ...
  def exists:Option[IEntityUI]= {
    ...
    var entity: Option[IEntityUI]= None
    while (en.hasMoreElements){
      entity= Some(en.nextElement.asInstanceOf[IEntityUI])
      if (entity.get.name.equals(nameField.getText) && entity.get.entityType.equals(Class.forName(typeButtonGroup.getSelection.getActionCommand))){
        return entity
      }
    }
    entity
