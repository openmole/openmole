...
class MoleSceneConverter extends Converter{
  override def marshal(o: Object,writer: HierarchicalStreamWriter,mc: MarshallingContext) = {
    ...
    molescene.manager.getTransitions.foreach(trans=> {
        ...
        writer.addAttribute("condition", trans.condition.get)
        writer.endNode
      })
  }
  ...
  override def unmarshal(reader: HierarchicalStreamReader,uc: UnmarshallingContext) =  {
    ...
    while (reader.hasMoreChildren) {
      ...
      n0 match {
        ...
        case "transitions"=> {
            val source = oslots.get(reader.getAttribute("source"))
            val target = islots.get(reader.getAttribute("target"))
            val condition = reader.getAttribute("condition")
            scene.manager.registerTransition(new TransitionUI(source.get, target.get, Some(condition)))
            scene.createEdge(scene.manager.capsuleViewID(source.get), scene.manager.capsuleViewID(target.get.capsuleView))           
          }
