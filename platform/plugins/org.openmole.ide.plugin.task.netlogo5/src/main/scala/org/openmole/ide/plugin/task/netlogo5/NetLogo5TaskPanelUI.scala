...
class NetLogo5TaskPanelUI(ndu: NetLogo5TaskDataUI) extends MigPanel("","[left]rel[grow,fill]","") with ITaskPanelUI{
  ...
  override def saveContent(name: String): ITaskDataUI = new NetLogo5TaskDataUI(name, workspaceTextField.text, nlogoTextField.text, launchingCommandTextArea.text)
}